...
def newDF():  # 新建一个dataframe文件
    return pd.DataFrame(columns=['desc_1', 'DKH_materiel_id', 'materiel_desc', 'norms', 'date', 'sfa_desc', 'sfa_id', 'amount', 'UnitPrice', 'sales_Money',
...
if __name__ == '__main__':
    ...
    priceDict = {}  # 单价
    materielDict = {}  # 商品简称
    dictionary = pd.read_excel('D:/FilesCenter/DKH-BottomTable/商品编码对照字典.xlsx', header=0, sheet_name=0, dtype=str)
    for irow in range(dictionary.shape[0]):
        priceDict[str(dictionary.loc[irow, '编码']).replace('.0', '')] = dictionary.loc[irow, '单价']
        materielDict[str(dictionary.loc[irow, '编码']).replace('.0', '')] = dictionary.loc[irow, 'materiel_alias']

    # gjcomDict = {}  # 高济平台
    # gjcompany = xlrd.open_workbook('C:\\Users\\Zeus\\Desktop\\autoSend\\2_大客户\\目标\\大客户_数据源.xlsx')  # 载入字典
    ...
    gjcomDict = {}  # 高济平台
    gjcompany = pd.read_excel('C:/Users/Zeus/Desktop/autoSend/2_大客户/目标/大客户_数据源.xlsx', header=0, sheet_name='高济目标公司', dtype=str)
    for jrow in range(gjcompany.shape[0]):
        gjcomDict[gjcompany.loc[jrow, '公司全称']] = gjcompany.loc[jrow, '平台']


    # k 为 0 时 导入大数据源数据; k 为 本月月份时 导入other文件夹数据

    k = input(">>>0upAll-thisMonth:")
    ...
    if k == "0":  # 上传每月大改数据
        ...
        if uploadType == 'A' or uploadType == 'a':  # 上传全部数据
            ...
            all_excel = get_excel("D:\\FilesCenter\\大客户数据源\\")  # 获取路径下全部xlsx文件
            alldf = newDF()
            for i in range(len(all_excel)):
                # 逐一读取EXCEL文件
            ...
            print(">  上传MySQLing: root -> dkh -> dkhfact")
            # 设置mysql连接引擎
            engine = create_engine('mysql+pymysql://root:123456@localhost:3306/dkh', encoding='utf-8', echo=False,
            ...
            for i in range(len(dfMerge)):
                if re.sub('\xa0', '', dfMerge.at[i, 'orgDataColumn'].replace(' ', '')) not in toList:
                    dfCreate = dfCreate.append(dfMerge.loc[[i]])
            dfCreate = dfCreate.reset_index(drop=True)

            # 增加客户英文前缀
            customerCname = ['老百姓', '海王', '益丰', '大参林', '国大', '全亿', '漱玉', '高济', '湖南千金', '湖南和盛', '湖南怀仁', '吉林亚泰', '江西华氏', '正和祥', '健之佳', '一心堂', '诚民天济',
                             '广东九州通', '河南张仲景', '四川太极', '益生天济', '柳州桂中', '国大湖北', '怡康', '唐人', '华润江苏', '江苏卫民', '山东金通', '青岛春天之星', '山东葆春堂', '聊城五洲']
            customerEname = ['LBX', 'HW', 'YF', 'DSL', 'GD', 'QY', 'SY', 'GJ', 'HNQJ', 'HNHS', 'HNHR', 'JLYT', 'JXHS', 'ZHS', 'JZJ', 'YXT', 'CMTJ',
                             'GDJZT', 'HNZZJ', 'SCTJ', 'YSTJ', 'LZGZ', 'GDHB', 'YK', 'TR', 'HRJS', 'JSWM', 'SDJT', 'QDCTZX', 'SDBCT', 'LCWZ']
            ...
            dfCreate.to_excel("D:\\FilesCenter\\DKH-BottomTable\\【Win】新增门店.xlsx")
            ...
            def dkhOrginalFiles(path):
                ...
                cprint(Year + '.' + Month + "删除旧数据" + str(delRowNum) + "行; 即将导入新数据" + str(selRowsNum) + "行; 新增数据" + str(
                    selRowsNum - delRowNum) + "行; 删除及整合数据耗时:" + strftime("%H:%M:%S", gmtime(
                    (endMakeTime - startMakeTime).seconds)), 'cyan', attrs=['bold', 'reverse', 'blink'])
                ...
                dfCreateNew.to_excel("D:\\FilesCenter\\DKH-BottomTable\\【Win】新增门店.xlsx")
                endOutTime = datetime.datetime.now()
                print(">> 导出新增门店耗时：" + strftime("%H:%M:%S", gmtime((endOutTime - startOutTime).seconds)))
    ...
    # 每日大客户数据上传(输入k数字是本月月份)
        ...
        cleanStartTime = datetime.datetime.now()  # 开始计时
        ...
        print(" > 高济")
        gjdf = newDF()  # 高济
        # xl = pd.ExcelFile("D:\\OTHER\\GJ"+str(month)+".xlsx")
        ...
        print(" > 其他")
        ordf = newDF()  # 其他
        columns = ['desc_1', 'DKH_materiel_id', 'materiel_desc', 'norms', 'date', 'sfa_desc', 'sfa_id', 'amount', 'UnitPrice', 'sales_Money', 'bz_UnitPrice',
                   'bz_sales_Money', 'customer', 'materiel_alias', 'dept_5', 'sfa_client_desc', 'state', 'client_id', 'client_desc', 'client_alias']
        ...
        # alldf.to_excel('C:/Users/Zeus/Desktop/alldf.xlsx', index=False)
        alldf['date'] = pd.to_datetime(alldf['date'], format='%Y/%m/%d').dt.date  # 规范日期格式
        alldfRowsNum = len(alldf)  # 整合数据行数

        cleanEndTime = datetime.datetime.now()

        print( "整合数据耗时:" + strftime("%H:%M:%S", gmtime((cleanEndTime - cleanStartTime).seconds)))

        # 上传前删除历史数据(按月)
        print('>> 【Win】删除历史数据中,请稍等片刻')

        delWinStartTime = datetime.datetime.now()

        connWin = pymysql.connect(
            host='localhost',
            port=3306,
            user='root',
            passwd='123456',
        ...
        cursorWin = connWin.cursor()  # 获取游标
        # 每年初注意修改
        sql_delectWin = "DELETE FROM DKHFACT WHERE month(date) = '" + str(month).zfill(2) + "' and year(date) = '2021'"
        cursorWin.execute(sql_delectWin)  # SQL语句删除当月旧数据
        delWinRowNum = cursorWin.rowcount  # 删除旧数据条数
        connWin.commit()  # 提交确认
        cursorWin.close()  # 关闭光标
        connWin.close()  # 关闭连接

        dtypedictWin = mapping_df_types(alldf)

        delWinEndTime = datetime.datetime.now()

        cprint("【Win】" + str(month) + "月旧数据" + str(delWinRowNum) + "行; 即将导入新数据" + str(alldfRowsNum) + "行; 新增数据" + str(
            alldfRowsNum - delWinRowNum) + "行; 删除旧数据耗时:" + strftime("%H:%M:%S", gmtime((delWinEndTime - delWinStartTime).seconds)), 'cyan',
               attrs=['bold', 'reverse', 'blink'])

        startWinUploadDBTime = datetime.datetime.now()
        # 【Win】上传主数据库
        print(" > 上传Win: root -> dkh -> dkhfact")
        # 连接mysql引擎
        engineWin = create_engine('mysql+pymysql://root:123456@localhost:3306/dkh', encoding='utf-8', echo=False,
                               pool_size=100, max_overflow=10, pool_timeout=100, pool_recycle=7200)
        alldf.to_sql('dkhfact', con=engineWin, dtype=dtypedictWin, index=False, if_exists='append')

        # 本月数据(用于钉钉机器人快速发送数据)
        print(" > 上传Win: root -> dkh -> ddmonth")
        # 连接mysql引擎
        ddEngineWin = create_engine('mysql+pymysql://root:123456@localhost:3306/dkh', encoding='utf-8', echo=False,
                                 pool_size=100, max_overflow=10, pool_timeout=100, pool_recycle=7200)
        alldf.to_sql('ddmonth', con=ddEngineWin, dtype=dtypedictWin, index=False, if_exists='replace')

        endWinUploadDBTime = datetime.datetime.now()
        print("【Win】上传数据库耗时:" + strftime("%H:%M:%S", gmtime((endWinUploadDBTime - startWinUploadDBTime).seconds)))


        # 上传前删除历史数据(按月)
        print('>> 【Linux】删除历史数据中,请稍等片刻')

        delLinuxStartTime = datetime.datetime.now()

        connLinux = pymysql.connect(
            host='192.168.20.241',
            port=3306,
            user='root',
            passwd='Powerbi#1217',
        ...
        cursorLinux = connLinux.cursor()  # 获取游标
        # 每年初注意修改
        sql_delectLinux = "DELETE FROM dkhfact WHERE month(date) = '" + str(month).zfill(2) + "' and year(date) = '2021'"
        cursorLinux.execute(sql_delectLinux)  # SQL语句删除当月旧数据
        delLinuxRowNum = cursorLinux.rowcount  # 删除旧数据条数
        connLinux.commit()  # 提交确认
        cursorLinux.close()  # 关闭光标
        connLinux.close()  # 关闭连接

        dtypedictLinux = mapping_df_types(alldf)

        delLinuxEndTime = datetime.datetime.now()
        cprint("【Linux】" + str(month) + "月旧数据" + str(delLinuxRowNum) + "行; 即将导入新数据" + str(alldfRowsNum) + "行; 新增数据" + str(
            alldfRowsNum - delLinuxRowNum) + "行; 删除旧数据耗时:" + strftime("%H:%M:%S", gmtime((delLinuxEndTime - delLinuxStartTime).seconds)), 'cyan',
               attrs=['bold', 'reverse', 'blink'])

        startLinuxUploadDBTime = datetime.datetime.now()
        # 【Linux】上传主数据库
        print(" > 上传Linux: root -> dkh -> dkhfact")
        # 连接mysql引擎
        engineLinux = create_engine('mysql+pymysql://root:Powerbi#1217@192.168.20.241:3306/dkh', encoding='utf-8', echo=False,
                               pool_size=100, max_overflow=10, pool_timeout=100, pool_recycle=7200)
        alldf.to_sql('dkhfact', con=engineLinux, dtype=dtypedictLinux, index=False, if_exists='append')

        endLinuxUploadDBTime = datetime.datetime.now()
        print("【Linux】上传数据库耗时:" + strftime("%H:%M:%S", gmtime((endLinuxUploadDBTime - startLinuxUploadDBTime).seconds)))
        ...
        for i in range(len(dfCreate)):
            for c, e in zip(customerCname, customerEname):
                if dfCreate.loc[i, "customer"] == c:
                    ...
                    pass
        dfCreate.to_excel("D:\\FilesCenter\\DKH-BottomTable\\【Win+Linux】新增门店.xlsx")
    ...
    cprint(">>>新增门店导出成功！ >> 记得维护标准门店ID哦 > ", 'magenta', attrs=['bold', 'reverse', 'blink'])
