...
cprint("【powerbi库】最新日期：" + str(nearestTime) + "; 即将导入" + str(missMonthHANA) + "时间段内的数据", 'magenta',
...
for iMonth in missMonthHANA:
    ...
    try:
        ...
        cprint(Year + "." + Month + "数据获取成功end！拜访表有" + str(len(dataBF)) + "行数据; 签退表有" + str(
            len(dataQT)) + "行数据; 耗时：" + strftime("%H:%M:%S",
                                                 gmtime((endGetHanaDataTime - startGetHanaDataTime).seconds)),
    ...
    try:
        ...
        cprint(Year + "." + Month + "拜访签退表合并成功！ 最后成功匹配的数据剩" + str(len(res)) + "行; 耗时：" + strftime("%H:%M:%S", gmtime(
    ...
    try:
        ...
        connDel.close()  # 关闭连接
        print("删除" + Year + "." + Month + "旧数据" + str(delRowNum) + "行")
        ...
        uploadSQL(uploadData, 'sfa', 'sfa_bf_fact')  # 上传数据
        print("upload数据库完成end！！！")
