'''
Author: zoodehao
Date: 2021-06-28 11:50:17
LastEditTime: 2021-09-03 11:27:19
FilePath: \PyCode\3_销管【整合】发送版.py
...
# 加载格式框架
# ----------------------------------------------------------------------------
framepath = 'C:/Users/Zeus/Desktop/autoSend/3_销管/目标/销管_格式框架.xlsx'
...
# ----------------------------------------------------------------------------
# 发货回款各大区目标
target = pd.read_excel('C:/Users/Zeus/Desktop/autoSend/3_销管/目标/销管_数据源.xlsx', sheet_name=0, header=0)
# 黄芪区域匹配维度
huangQi = pd.read_excel('C:/Users/Zeus/Desktop/autoSend/3_销管/目标/销管_数据源.xlsx', sheet_name=1, header=0)
...
def get_matZSD082(ZSD082):
    ...
    cursorZSD082.execute("""WITH AA AS (SELECT MATNR, MAKTX
                            ...
                            BB AS ( SELECT DATE_SQL, MATNR, LFIMG3, WJHJE, ERDAT
                            FROM "ECC_BI"."ZEXHSUSTED"
                            WHERE ZZPART1_T_S = '草晶华销售事业部' AND YEAR(DATE_SQL) = :1)
                            SELECT AA.MAKTX, BB.LFIMG3, BB.WJHJE, BB.ERDAT, BB.DATE_SQL
                            FROM BB
...
# 4. 欠货表ZSD082
misgoods = pd.DataFrame(get_matZSD082(conn), columns=['品名', '数量', '欠货金额', '开始欠货时间', '预计出货时间'])
...
columnWidth = [15, 17, 17, 17, 40, 17, 70]
...
# 获取最新日期数据
misgoods = misgoods[misgoods["预计出货时间"] == max(misgoods["预计出货时间"])]
# 透视原欠货表
...
# ----------------------------------------------------------------------------
try:
    workbook.save('C:/Users/Zeus/Desktop/autoSend/3_销管/销管_正式发送文件.xlsx')
    print('文件储存完毕！！！')
...
if __name__ == '__main__':
    ...
    deleteOldFiles('C:/Users/Zeus/Desktop/autoSend/3_销管/Pictures/')  # 清空文件夹历史文件
    ...
    fileFullPath = 'C:/Users/Zeus/Desktop/autoSend/3_销管/销管_正式发送文件.xlsx'
    savePictuePath = 'C:/Users/Zeus/Desktop/autoSend/3_销管/Pictures/'
