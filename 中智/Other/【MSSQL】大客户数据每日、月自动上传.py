...
if __name__ == '__main__':
    ...
    if k == "0":  # 上传每月大改数据
        ...
        if uploadType == 'A' or uploadType == 'a':  # 上传全部数据
            ...
            print(" > 上传MSSQLing: localhost -> SY -> DKHFACT")
            # 设置mssql连接引擎
            engine = create_engine('mssql+pymssql://sa:123456@localhost:1433/SY', encoding='utf-8', echo=False,
                                   pool_size=100, max_overflow=10, pool_timeout=100, pool_recycle=7200)
            alldf.to_sql('DKHFACT', conn=engine, dtype=dtypedict, index=False, if_exists='replace')
            ...
            for i in range(len(dfCreate)):
                for c, e in zip(customerCname, customerEname):
                    if dfCreate.loc[i, "customer"] == c:
                        ...
                        pass
            dfCreate.to_excel("D:\\FilesCenter\\DKH-BottomTable\\【MSSQL】新增门店.xlsx")
            ...
            def dkhOrginalFiles(path):
                ...
                engine = create_engine('mssql+pymssql://sa:123456@localhost:1433/SY', encoding='utf-8', echo=False,
                                       pool_size=100, max_overflow=10, pool_timeout=100, pool_recycle=7200)
                dfCreate.to_sql('DKHFACT', conn=engine, dtype=dtypedict, index=False, if_exists='append')
                ...
                for i in range(len(dfCreateNew)):
                    for c, e in zip(customerCname, customerEname):
                        if dfCreateNew.loc[i, "customer"] == c:
                            ...
                            pass
                dfCreateNew.to_excel("D:\\FilesCenter\\DKH-BottomTable\\【MSSQL】新增门店.xlsx")
                endOutTime = datetime.datetime.now()
                print(">> 导出新增门店耗时：" + strftime("%H:%M:%S", gmtime((endOutTime - startOutTime).seconds)))
    ...
    # 每日大客户数据上传(输入k数字是本月月份)
        ...
        # engine = create_engine('mssql+pymssql://sa:123456@localhost:1433/SY', encoding='utf-8', echo=False,
        #                        pool_size=100, max_overflow=10, pool_timeout=100, pool_recycle=7200)
        # alldf.to_sql('DKHFACT', con=engine, dtype=dtypedict, index=False, if_exists='append')

        # 本月数据(用于钉钉机器人快速发送数据)
        print(" > 上传MSSQLing: localhost -> SY -> ddmonth")
        # 连接mssql引擎
        ddEngine = create_engine('mssql+pymssql://sa:123456@localhost:1433/SY', encoding='utf-8', echo=False,
                                 pool_size=100, max_overflow=10, pool_timeout=100, pool_recycle=7200)
        alldf.to_sql('ddmonth', con=ddEngine, dtype=dtypedict, index=False, if_exists='replace')
