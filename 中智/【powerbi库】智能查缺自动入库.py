...
clearFiles = 'C:/Users/Zeus/AppData/Local/Temp/gen_py/3.9/'  # 需清空文件路径
shutil.rmtree(clearFiles)  # 递归删除文件夹
createFiles = 'C:/Users/Zeus/AppData/Local/Temp/gen_py/3.9/'  # 创建文件夹路径
os.mkdir(createFiles)  # 创建3.8空文件夹

connPbi = pymysql.connect(host='localhost',
                          port=3306,
                          user='root',
                          passwd='123456',
                          db='powerbi',
                          charset='utf8')
cursorPbi = connPbi.cursor()
executePbi = "SELECT MAX(Date) FROM pbi_log"
cursorPbi.execute(executePbi)
timeStr = cursorPbi.fetchall()[0]
nearestTimePbi = str(timeStr)[2:13].replace(",", "-").replace(" ", "")  # 最新日期
connPbi.commit()  # 提交确认
cursorPbi.close()  # 关闭光标
connPbi.close()  # 关闭连接
todayPbi = datetime.datetime.now()  # 本日
todayBeforePbi = (todayPbi + datetime.timedelta(days=-1)).strftime("%Y-%m-%d")  # 前一天

missDate = getEveryDay(nearestTimePbi, todayBeforePbi)
cprint("【powerbi库】最新日期：" + str(nearestTimePbi) + "; 即将导入" + str(missDate) + "时间段内的数据", 'magenta', attrs=['bold', 'reverse', 'blink'])
for iDate in missDate:
    everyStartTime = datetime.datetime.now()
    Year = iDate[0:4]  # 年
    Month = iDate[5:7]  # 月
    Day = iDate[8:10]  # 日
    ...
    ap.ShellExecute(0, 'open',
                    'C:\\Users\\Zeus\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Windows PowerShell\\Windows PowerShell',
                    '', '', 1)  # 打开Windows PowerShell
    time.sleep(2)
    # pg.click(267, 242)  # 点击屏幕
    inputCode = 'login-powerBI -environment China'
    passWord = 'Zeuspw0760'
    executeCode = "$activities = Get-PowerBIActivityEvent -StartDateTime '" + str(
        Year
    ) + "-" + str(Month) + "-" + str(Day) + "T00:00:00' -EndDateTime '" + str(
        Year
    ) + "-" + str(Month) + "-" + str(
        Day
    ) + "T23:59:59' | ConvertFrom-Json >D:\\FilesCenter\\PowerBi_Log_To_xlsx\\PowerBi_Log_To_Txt\\pbidata" + str(
        Year) + "." + str(Month) + "." + str(Day) + ".txt"
    closePowerShell = 'exit'
    cp.copy(inputCode)  # 复制内容
    pg.hotkey('ctrl', 'v')  # 粘贴
    pg.press('enter')  # 回车
    # time.sleep(2)  # 等待选择账号前的是
    # pg.click(763, 563,clicks=1,duration=0.2)  # 页脚发生错误时, 点击是
    time.sleep(12)  # 等待选择账号前的是
    # pg.click(749, 477, clicks=2, interval=1)  # 点击是2次
    pg.press('space')
    time.sleep(1)
    pg.press('space')
    ...
    pg.hotkey('ctrl', 'v')  # 粘贴
    pg.press('enter')  # 回车
    time.sleep(25)  # 等待下载文件的时间
    os.chdir('D:/FilesCenter/PowerBi_Log_To_xlsx/')  # 定位目录根
    downLoadTxtFile = glob.glob('PowerBi_Log_To_Txt/pbidata' + str(Year) + "." + str(Month) + "." + str(Day) + ".txt")  # 定位文件夹下是否已经存在下载文件
    ...
    if len(downLoadTxtFile) == 1:  # 存在日志数据执行以下程序
        ...
        def uploadDB():
            ...
            '''
            with open('PowerBI_Other/123.txt', 'r',
                      encoding='utf-8') as d1, open('PowerBI_Other/456.txt', 'w', encoding='utf-8') as d2:
                d2.write(''.join(d1.readlines()[1:]))

            strList = []
            with open('PowerBI_Other/456.txt', 'r', encoding='utf-8') as d3, open('PowerBI_Other/789.txt', 'w', encoding='utf-8') as d4:
                ...
                for i in range(len(strList)):
                    if strList[i][0:9] == 'UserAgent':
                        if strList[i + 1][0:9] != '         ':
                            ...
                            if strList[i + 2][0:9] != '         ':
                                d4.write(
                                    str(strList[i]).rstrip() +
                                    str(strList[i + 1]).lstrip())
                            else:
                                d4.write(
                                    str(strList[i]).rstrip() +
                                    str(strList[i + 1]).lstrip())
                        ...
                        if strList[i][0:12] == 'CreationTime':
                            d4.write(
                                strList[i] + "Date               : " +
                                re.sub('\\s+', '', strList[i]).strip()[13:23] +
                                "\n" + "Time               : " +
                                re.sub('\\s+', '', strList[i]).strip()[24:32] +
                                "\n" + "DateTime           : " +
                                re.sub('\\s+', '', strList[i]).strip()[13:23] +
                                " " +
                                re.sub('\\s+', '', strList[i]).strip()[24:32] +
            ...
            colName = [
                'Id', 'RecordType', 'CreationTime', 'Date', 'Time', 'DateTime',
                'Operation', 'OrganizationId', 'UserType', 'UserKey',
                'Workload', 'UserId', 'ClientIP', 'UserAgent', 'Activity',
                'ItemName', 'WorkSpaceName', 'DatasetName', 'ReportName',
                'WorkspaceId', 'ObjectId', 'DatasetId', 'ReportId',
                'IsSuccess', 'ReportType', 'RequestId', 'ActivityId',
                'DistributionMethod', 'ConsumptionMethod', 'DashboardName',
                'DashboardId', 'Datasets', 'SharingInformation',
            ...
            exitOldExcel = glob.glob("PowerBi_Log_To_xls/pbidata" + str(Year) + "." + str(Month) + "." + str(Day) + ".xls")
            if len(exitOldExcel) == 1:
                ...
                exitNewExcel = glob.glob("PowerBi_Log_To_xls/pbidata" + str(Year) + "." + str(Month) + "." + str(Day) + ".xls")
            ...
            time.sleep(0.5)
            # Excel格式转换：.xls ---> .xlsx
            fileName = "D:\\FilesCenter\\PowerBi_Log_To_xlsx\\PowerBi_Log_to_xls\\pbidata" + str(Year) + "." + str(Month) + "." + str(Day) + ".xls"
            # excel = win32.gencache.DispatchEx('Excel.Application')
            ...
            def open_Excel():
                try:
                    book = xlrd.open_workbook("D:/FilesCenter/PowerBi_Log_To_xlsx/PowerBi_Log_to_xls/pbidata"
                                              + str(Year) + "." + str(Month) + "." + str(Day) + ".xlsx")
                except BaseException:
                    print("读取数据出错！！！")
                try:
            ...
            def insert_Data():
                ...
                for i in range(1, row_Num):
                    ...
                    value = (row_data[0], row_data[1], row_data[2],
                             ...
                             row_data[9], row_data[10], row_data[11],
                             row_data[12], row_data[13], row_data[14],
                             row_data[15], row_data[16], row_data[17],
                             row_data[18], row_data[19], row_data[20],
                             row_data[21], row_data[22], row_data[23],
                             row_data[24], row_data[25], row_data[26],
                             row_data[27], row_data[28], row_data[29],
                             row_data[30], row_data[31], row_data[32],
                             row_data[33], row_data[34])
                ...
                database.commit()
                cursor.close()  # 关闭连接
                everyEndTime = datetime.datetime.now()
                cprint(">>>" + str(Year) + "." + str(Month) + "." + str(Day) + "删除旧数据" + str(delRowNum) + "行;" + "导入新数据" +
                       str(row_Num - 1) + "行; 新增数据" + str(row_Num - 1 - delRowNum) + "行; 耗时：" +
                       strftime("%H:%M:%S", gmtime((everyEndTime - everyStartTime).seconds)), 'cyan', attrs=['bold', 'reverse', 'blink'])
        ...
        try:
            ...
            print("出错！！！【删除 Path 下文件】", e)  # Path = C:\Users\Zeus\AppData\Local\Temp\gen_py\3.9\
    else:
