...
LastEditTime: 2021-09-26 20:10:24
FilePath: \PyCode\6_PK100定制报表.py
...
def get_matFHCX(FHCX):
    ...
    cursorFHCX.execute("""
                       ...
                       WHERE ERP_SALES_ORG IN ('2001', '2008') AND ZYEAR = '2021' -- 纯销日期筛选范围
                       ...
                       WHERE ZZPART1_T_S LIKE N'%草晶华销售事业部%' AND LFDAT != '' AND BEZEI2 != '领用订单' AND YEAR(LFDAT) = '2021' -- 发货日期筛选范围
...
allData.to_sql('PK100', con=enginePK100, dtype=dtypedictPK100, index=False, if_exists='replace')
# allData.to_excel('C:/Users/zoodehao/Desktop/AutoSend/6_百日PK/百日PK数据源-' + str(globalYear) + str(globalMonth).zfill(2) + str(globalDay).zfill(2) + '.xlsx', index=False)
